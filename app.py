...
def home():
    if "email" in session:
        ...
        return render_template("index.html", lab_name=lab_name, lab_description=lab_description, lab_url=lab_url)
...
def login():
    ...
    if request.method == 'POST':
        ...
        cursor.execute("SELECT * FROM basic_users WHERE basic_user_email = %s AND basic_user_password = %s", (email, password))
        data = cursor.fetchone()
        if data:
